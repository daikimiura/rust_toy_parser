...
type AstNode = Annot<AstNodeKind>;

impl AstNode {
    fn num(n: u64, loc: Loc) -> Self {
        Self::new(AstNodeKind::Num(n), loc)
    }

    fn uniop(op: UniOp, e: AstNode, loc: Loc) -> Self {
        Self::new(AstNodeKind::UniOp { op, e: Box::new(e) }, loc)
    }

    fn binop(op: BinOp, l: AstNode, r: AstNode, loc: Loc) -> Self {
        Self::new(
            AstNodeKind::BinOp {
                op,
                l: Box::new(l),
...
fn parse(tokens: Vec<Token>) -> Result<AstNode, ParseError> {
...
fn parse_expr<Tokens>(tokens: &mut Peekable<Tokens>) -> Result<AstNode, ParseError>
where
...
fn parse_expr3<Tokens>(tokens: &mut Peekable<Tokens>) -> Result<AstNode, ParseError>
where
    Tokens: Iterator<Item = Token>,
{
...
fn parse_expr2<Tokens>(tokens: &mut std::iter::Peekable<Tokens>) -> Result<AstNode, ParseError>
where
    Tokens: Iterator<Item = Token>,
{
...
fn parse_left_binop<Tokens>(
    ...
    subexpr_parser: fn(&mut Peekable<Tokens>) -> Result<AstNode, ParseError>,
...
fn parse_expr1<Tokens>(tokens: &mut Peekable<Tokens>) -> Result<AstNode, ParseError>
where
    Tokens: Iterator<Item = Token>,
{
    match tokens.peek().map(|tok| tok.value) {
        Some(TokenKind::Plus) | Some(TokenKind::Minus) => {
            ...
            Ok(AstNode::uniop(op, e, loc))
        }
        _ => parse_atom(tokens),
    }
}

fn parse_atom<Tokens>(tokens: &mut Peekable<Tokens>) -> Result<AstNode, ParseError>
where
    Tokens: Iterator<Item = Token>,
{
    tokens
        ...
        .ok_or(ParseError::Eof)
        .and_then(|tok| match tok.value {
            TokenKind::Number(n) => Ok(AstNode::new(AstNodeKind::Num(n), tok.loc)),
            TokenKind::LParen => {
...
fn main() {
    ...
    loop {
        ...
        if let Some(Ok(line)) = lines.next() {
            let ast = match line.parse::<AstNode>() {
                Ok(ast) => ast,
                Err(e) => {
...
fn test_parser() {
    ...
    assert_eq!(
        ...
        Ok(AstNode::binop(
            ...
            AstNode::binop(
                BinOp::add(Loc(2, 3)),
                AstNode::num(1, Loc(0, 1)),
                AstNode::binop(
                    BinOp::mult(Loc(6, 7)),
                    AstNode::num(2, Loc(4, 5)),
                    AstNode::num(3, Loc(8, 9)),
                    Loc(4, 9)
                ),
                Loc(0, 9)
            ),
